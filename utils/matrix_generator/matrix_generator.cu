...
void print_help() {
  ...
  cout
      << "Generate a matrix stored at [DEST] with [N] rows and [M] columns\n\n";
...
vector<int> matrix_generator(int N, int M) {
  ...
  cudaMemcpy(d_matrix, matrix.data(), N * M * sizeof(int),
  ...
  cudaMemcpy(matrix.data(), d_matrix, N * M * sizeof(int),
...
int main(int argc, char *argv[]) {
  ...
  cout << "Generating matrix with " << N << " rows and " << M << " columns"
       << endl;
